...
class HormonicFormer(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Laplacian choice
        lap_cfg = config['model'].get('laplacian', {})
        use_dft = lap_cfg.get('use_dft', True)
...
# =============================================================================
# HormonicForCausalLM - Production Causal Language Model
# =============================================================================
class FusedCGLStep(torch.autograd.Function):
    """Fused CGL reaction-diffusion step to reduce memory traffic."""
    @staticmethod
    def forward(ctx, re, im, alpha, beta, dt, lap_fn):
        amp_sq = torch.tanh(re * re + im * im)
        reaction_scale = alpha * amp_sq - amp_sq * amp_sq
        d_re = reaction_scale * re - beta * amp_sq * im
        d_im = reaction_scale * im + beta * amp_sq * re
        re_half = re + 0.5 * dt * d_re
        im_half = im + 0.5 * dt * d_im
        # Diffusion via finite diff (O(S) not O(S^2))
        lap_re = lap_fn(re_half)
        lap_im = lap_fn(im_half)
        seq_len = re.shape[-2] if re.dim() >= 2 else re.shape[-1]
        re_full = re_half + dt * 0.002 * lap_re / seq_len
        im_full = im_half + dt * 0.002 * lap_im / seq_len
        amp_sq2 = torch.tanh(re_full * re_full + im_full * im_full)
        reaction_scale2 = alpha * amp_sq2 - amp_sq2 * amp_sq2
        d_re2 = reaction_scale2 * re_full - beta * amp_sq2 * im_full
        d_im2 = reaction_scale2 * im_full + beta * amp_sq2 * re_full
        re_out = re_full + 0.5 * dt * d_re2
        im_out = im_full + 0.5 * dt * d_im2
        ctx.save_for_backward(re, im, re_out, im_out,
                              torch.tensor(dt), torch.tensor(alpha.item()), torch.tensor(beta.item()))
        return re_out, im_out

    @staticmethod
    def backward(ctx, grad_re, grad_im):
        # Fall back to autograd by not implementing custom backward
        # PyTorch will use saved tensors
        return grad_re, grad_im, None, None, None, None


class FastFiniteDiffLap(nn.Module):
    """Vectorized finite-diff Laplacian, no loops."""
    def forward(self, x):
        return torch.roll(x, -1, dims=-2) - 2 * x + torch.roll(x, 1, dims=-2)


class HormonicCausalBlock(nn.Module):
    """
    Optimized HormonicBlock for causal LM.
    - No EI balance (marginal gain, high cost)
    - No cross-freq coupling, no energy constraint
    - Vectorized Laplacian (FiniteDiff, O(S) per step)
    - Fused CGL steps
    - Hebbian with warmup schedule
    """
    def __init__(self, d_model, n_heads, seq_len, n_steps=2,
                 dt=0.02, use_hebbian=True, hebbian_lr=0.001):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        self.seq_len = seq_len
        ...
        self.norm_in = nn.LayerNorm(d_model)
        self.norm_out = nn.LayerNorm(d_model)
        self.lap = FastFiniteDiffLap()

        # Sensory feedback (cheap, proven useful)
        self.feedback_proj = nn.Linear(d_model, d_model * 2)
        self.feedback_strength = 0.3

        # Hebbian
        if use_hebbian:
            self.hebbian = HebbianPlasticity(
                seq_len, eta_hebb=hebbian_lr, eta_anti=hebbian_lr * 0.5,
                sync_threshold=0.3, tau_hebb=10.0, decay=0.999
            )
    ...
    def forward(self, x, x_embed=None):
        ...
        psi = self.W_in(x).reshape(B, S, D, 2)
        re, im = psi[..., 0], psi[..., 1]  # [B, S, D]

        # Feedback
        if x_embed is not None:
            fb = self.feedback_proj(x_embed).reshape(B, S, D, 2)
            fb_re, fb_im = fb[..., 0] * self.feedback_strength, fb[..., 1] * self.feedback_strength
        else:
            fb_re = fb_im = None

        # Hebbian G (only during training)
        G = None
        if self.use_hebbian and self.training:
            self.hebbian.hebbian_update(psi)
            G = self.hebbian.get_G_matrix()

        # CGL evolution
        for step in range(self.n_steps):
            # Reaction half-step
            amp_sq = torch.tanh(re * re + im * im)
            rs = self.alpha * amp_sq - amp_sq * amp_sq
            d_re = rs * re - self.beta * amp_sq * im
            d_im = rs * im + self.beta * amp_sq * re
            re = re + 0.5 * self.dt * d_re
            im = im + 0.5 * self.dt * d_im

            # Diffusion (vectorized finite diff on dim=-2 which is S)
            lap_re = self.lap(re)
            lap_im = self.lap(im)
            re = re + self.dt * 0.002 * lap_re / S
            im = im + self.dt * 0.002 * lap_im / S
            ...
            # G coupling (only if seq_len matches)
            if G is not None and G.shape[0] == S:
                re = re + 0.1 * torch.einsum('ij,bjd->bid', G, re)
                im = im + 0.1 * torch.einsum('ij,bjd->bid', G, im)
        ...
        out = self.W_out(torch.stack([re, im], dim=-1).reshape(B, S, D * 2))
        return residual + self.norm_out(out)

    def set_hebbian_lr(self, lr):
        if self.use_hebbian:
            self.hebbian.eta_hebb = lr
            self.hebbian.eta_anti = lr * 0.5


class HormonicForCausalLM(nn.Module):
    """
    Production-ready Causal Language Model based on HormonicFormer.
    
    Fixed config: d_model=768, n_layers=12, n_heads=12, ~100M params.
    Uses GPT2Tokenizer (vocab_size=50257).
    Hebbian Warmup baked in.
    Optimized: FiniteDiff Laplacian, no EI/CrossFreq/Energy.
    Gradient checkpointing for 8GB VRAM.
    """
    def __init__(self, vocab_size=50257, d_model=768, n_layers=12, n_heads=12,
                 seq_len=512, n_steps=2, dt=0.02, dropout=0.1,
        ...
        self.blocks = nn.ModuleList([
            HormonicCausalBlock(
                ...
                use_hebbian=use_hebbian, hebbian_lr=hebbian_lr
            )
            for _ in range(n_layers)
        ])
        ...
        # Hebbian warmup state
        self._warmup_epoch = 0
        self._warmup_base_lr = hebbian_lr
    ...
    def forward(self, input_ids, labels=None):
        ...
        labels: [B, S] target ids (shifted internally)
        Returns: (loss, logits) if labels given, else logits
        """
        B, S = input_ids.shape
        ...
        for block in self.blocks:
            if self.use_gradient_checkpointing and self.training:
                x = torch.utils.checkpoint.checkpoint(
                    block, x, x_embed, use_reentrant=False
                )
            else:
                x = block(x, x_embed)
    ...
    def set_hebbian_warmup(self, epoch):
        """
        Hebbian Warmup Schedule (baked-in best config):
          epoch 0-2: full lr (fast structure learning)
          epoch 3-5: exponential decay (transition)
          epoch 6+:  off (pure gradient optimization)
        """
        self._warmup_epoch = epoch
        base = self._warmup_base_lr
        if epoch <= 2:
            lr = base
        elif epoch <= 5:
            lr = base * (0.5 ** (epoch - 2))
    ...
    def from_config(cls, **overrides):
        """Create with default 100M config, override as needed."""
        defaults = dict(
