...
def extract_diagnostics(checkpoint_path):
    ...
    try:
        ...
        print("\n" + "="*60)
        print("Neuromodulation Diagnostics")
        print("="*60)
        
        # STP Efficacy
        if 'stp_efficacy_mean' in diagnostics:
            print(f"STP Efficacy (u·r): {diagnostics['stp_efficacy_mean']:.4f}")
        
        # Homeostatic Gain
        if 'homeo_gain_mean' in diagnostics:
            print(f"Homeostatic Gain: {diagnostics['homeo_gain_mean']:.4f} ± {diagnostics.get('homeo_gain_std', 0):.4f}")
        
        # DA
        if 'da' in diagnostics:
            print(f"DA (Dopamine): {diagnostics['da']:.4f}")
        
        # CB
        if 'cb' in diagnostics:
            print(f"CB (Cortisol): {diagnostics['cb']:.4f}")
        
        # G Sparsity
        if 'G_sparsity' in diagnostics:
            sparsity = diagnostics['G_sparsity']
            print(f"G Sparsity: {sparsity:.2%} ({sparsity*100:.1f}% pruned)")
            print(f"G Alive: {(1-sparsity)*100:.2f}%")
