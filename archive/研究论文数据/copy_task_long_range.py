"""
HormonicFormer 长程序列建模 - Copy Task扩展
序列长度: 1024, 2048, 4096
对比: HormonicFormer vs Transformer vs LSTM
"""
...
# ============== 数据生成 ==============
def generate_copy_data(seq_len, vocab_size, batch_size):
    """生成Copy Task数据"""
...
def train_model(model, seq_len, vocab_size, epochs=10, batch_size=1):
    """训练模型并返回结果"""
    ...
    for epoch in range(epochs):
        ...
        # 每个epoch训练100个batch
        for _ in range(100):
            src, tgt = generate_copy_data(seq_len, vocab_size, batch_size)
        ...
        model.eval()
        with torch.no_grad():
            src, tgt = generate_copy_data(seq_len, vocab_size, 10)
            src, tgt = src.to(device), tgt.to(device)
            output = model(src)
        ...
        if epoch % 2 == 0:
            print(f"  Epoch {epoch}: Loss={avg_loss:.4f}, Acc={acc:.4f}, Time={epoch_time:.2f}s")
...
def run_experiment(seq_lengths=[128, 256, 512, 1024, 2048, 4096]):
    """运行完整实验"""
    
    print("=" * 70)
    print("HormonicFormer 长程序列建模 - Copy Task扩展")
    print("=" * 70)
    ...
    for seq_len in seq_lengths:
        ...
        try:
            ...
            result = train_model(model, seq_len, vocab_size, epochs=10, batch_size=1)
            results[seq_len]['hormonic'] = result
            print(f"  [OK] 完成: Final Acc={result['final_acc']:.4f}, Total Time={result['total_time']:.2f}s")
    ...
    # 保存结果
    output_file = f'C:/Users/MR/Desktop/论文/关于场物理的神经框架/研究论文数据/copy_long_range_results_{datetime.now().strftime("%m%d_%H%M")}.json'
